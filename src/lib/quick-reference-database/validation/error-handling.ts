...
export class ValidationErrorHandler {
  ...
  handleValidationError(
    ...
    if (error instanceof Error) {
      ...
      // Defensive: ensure `errors` is an array before joining
      const errs: string[] = Array.isArray(error?.errors) ? error.errors : []
      errorMessage = errs.join('; ')
      // Pass a normalized object to the message creator to avoid runtime errors
      userMessage = this.createUserFriendlyValidationMessage({ ...error, errors: errs }, context)
    }
  ...
  private createUserFriendlyValidationMessage(result: ValidationResult, context: ErrorContext): string {
    // Defensive checks: `errors` may be undefined or not an array
