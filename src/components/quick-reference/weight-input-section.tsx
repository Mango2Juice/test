...
import { useAccessibleId } from '@/lib/utils/accessibility/ids'
...
export function WeightInputSection({ audience, disabled }: WeightInputSectionProps) {
  ...
  const inputId = useAccessibleId('weight-input')
  const descriptionId = useAccessibleId('weight-description')
  ...
  const weightDescription = AriaLabels.weightInputDescription(audience)
