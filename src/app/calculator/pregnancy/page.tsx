// src/app/calculator/pregnancy/page.tsx
'use client'

import { Info, RefreshCw } from 'lucide-react'
import { PregnancyInputForm } from '@/components/calculator/pregnancy/pregnancy-input-form'
import { PregnancyResults } from '@/components/calculator/pregnancy/pregnancy-results'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { usePregnancyCalculator } from '@/hooks/use-pregnancy-calculator'

/**
 * Render the Pregnancy Due Date Calculator page.
 *
 * Displays a two-column layout with an input card for clinical data and a results area.
 * The results area shows pregnancy details when available or an informational placeholder prompting for LMP input.
 *
 * @returns The JSX element for the pregnancy due date calculator page
 */
export default function PregnancyCalculatorPage() {
  const {
    lmpDate,
    setLmpDate,
    ultrasoundDate,
    setUltrasoundDate,
    gaWeeks,
    setGaWeeks,
    gaDays,
    setGaDays,
    pregnancyInfo,
    handleReset,
  } = usePregnancyCalculator()

  return (
    <div className='w-full max-w-4xl mx-auto pb-24'>
      <div className='mb-8 hidden md:block'>
        <h1 className='text-3xl font-bold tracking-tight'>Pregnancy Due Date Calculator</h1>
      </div>

      <div className='grid grid-cols-1 md:grid-cols-2 gap-8 items-start'>
        <Card>
          <CardHeader>
            <CardTitle>Enter Clinical Data</CardTitle>
          </CardHeader>
          <CardContent className='space-y-6'>
            <PregnancyInputForm
              lmpDate={lmpDate}
              setLmpDate={setLmpDate}
              ultrasoundDate={ultrasoundDate}
              setUltrasoundDate={setUltrasoundDate}
              gaWeeks={gaWeeks}
              setGaWeeks={setGaWeeks}
              gaDays={gaDays}
              setGaDays={setGaDays}
            />
            <Button variant='outline' onClick={handleReset} className='w-full'>
              <RefreshCw className='mr-2 h-4 w-4' />
              Reset Calculator
            </Button>
          </CardContent>
        </Card>

        <div className='space-y-6'>
          {pregnancyInfo != null ? (
            <PregnancyResults pregnancyInfo={pregnancyInfo} />
          ) : (
            <Card>
              <CardContent className='p-6 text-center text-muted-foreground'>
                <Info className='mx-auto w-12 h-12 mb-4' />
                <p>Enter the LMP date to see pregnancy details.</p>
              </CardContent>
            </Card>
          )}
        </div>
      </div>
    </div>
  )
}